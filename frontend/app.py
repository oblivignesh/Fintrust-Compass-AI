...
Tabs:
  1. 💬 Chat Assistant  — RAG chat with streaming answers
  2. ✅ Eligibility Checker — Structured product eligibility check with HITL review
"""
...
tab_chat, tab_elig, tab_checklist, tab_calc, tab_compare = st.tabs(
    ["💬 Chat Assistant", "✅ Eligibility Checker", "📋 Document Checklist",
     "🧮 Fee & Interest Calculator", "⚖️ Policy Comparison"]
)
...
with tab_chat:
    st.caption("Ask any question about FinTrust Bank policies — loans, deposits, accounts, cards, compliance, or digital banking.")

    # Scrollable message history — keeps input pinned naturally at the bottom
    ...
    # Chat input sits naturally below the scrollable container
            ...
            with st.chat_message("assistant"):
                ...
                try:
                    with requests.post(
                        ...
                        json={"question": active_prompt, "conversation_id": st.session_state.conversation_id},
                        stream=True,
                        timeout=120,
                    ) as resp:
...
with tab_elig:
    st.subheader("✅ Product Eligibility Checker")
    st.caption(
    ...
    # ── Fields per product category ──────────────────────────────────────────
    if selected_product in LOAN_PRODUCTS:
        with c1:
            profile["age"]              = st.number_input("Age (years)", 18, 80, 35)
            profile["employment_type"]  = st.selectbox("Employment Type", ["Salaried", "Self-Employed Professional", "Self-Employed Business", "Pensioner"])
        with c2:
            profile["monthly_income"]   = st.number_input("Monthly Income (₹)", 0, 10_000_000, 75_000, step=5_000)
            profile["cibil_score"]      = st.number_input("CIBIL Score", 300, 900, 720)
        with c3:
            profile["loan_amount"]      = st.number_input("Loan Amount Required (₹)", 0, 100_000_000, 2_500_000, step=100_000)
            profile["loan_tenure_years"]= st.number_input("Desired Tenure (years)", 1, 30, 15)

        if selected_product == "home_loan":
            c4, c5 = st.columns(2)
            with c4:
                profile["property_value"]   = st.number_input("Property Value (₹)", 0, 100_000_000, 4_000_000, step=100_000)
                profile["property_type"]    = st.selectbox("Property Type", ["Under Construction", "Ready to Move", "Resale", "Plot + Construction"])
            with c5:
                profile["co_applicant"]     = st.selectbox("Co-Applicant?", ["No", "Yes — Spouse", "Yes — Parent", "Yes — Other"])
                profile["existing_loans"]   = st.selectbox("Existing Loans?", ["None", "1 Loan", "2+ Loans"])

        elif selected_product == "vehicle_loan":
            c4, c5 = st.columns(2)
            with c4:
                profile["vehicle_type"]     = st.selectbox("Vehicle Type", ["New Car", "Used Car (< 3 yrs)", "Used Car (3-7 yrs)", "Two Wheeler", "Commercial Vehicle"])
                profile["vehicle_value"]    = st.number_input("Vehicle On-Road Price (₹)", 0, 10_000_000, 800_000, step=50_000)
            with c5:
                profile["down_payment"]     = st.number_input("Down Payment (₹)", 0, 10_000_000, 160_000, step=10_000)
                profile["driving_license"]  = st.selectbox("Valid Driving License?", ["Yes", "No"])

        elif selected_product == "personal_loan":
            c4, c5 = st.columns(2)
            with c4:
                profile["company_category"] = st.selectbox("Employer Category", ["Govt / PSU", "Listed Company", "Private Ltd", "Partnership / Proprietorship"])
                profile["work_experience_years"] = st.number_input("Total Work Experience (years)", 0, 40, 5)
            with c5:
                profile["existing_emi"]     = st.number_input("Existing Monthly EMI obligations (₹)", 0, 500_000, 0, step=1_000)
                profile["purpose"]          = st.selectbox("Loan Purpose", ["Medical Emergency", "Education", "Travel", "Home Renovation", "Wedding", "Other"])

    elif selected_product in DEPOSIT_PRODUCTS:
        with c1:
            profile["customer_type"]    = st.selectbox("Customer Type", ["Resident Individual", "NRI", "HUF", "Company / Trust"])
            profile["age"]              = st.number_input("Age (years)", 0, 120, 35)
        with c2:
            profile["deposit_amount"]   = st.number_input("Deposit Amount (₹)", 0, 100_000_000, 50_000, step=1_000)
            profile["tenure_months"]    = st.number_input("Desired Tenure (months)", 1, 120, 12)
        with c3:
            profile["kyc_complete"]     = st.selectbox("KYC Complete?", ["Yes", "No", "Partial"])
            profile["existing_account"] = st.selectbox("Existing FinTrust Account?", ["Yes", "No"])

        if selected_product == "recurring_deposit":
            profile["monthly_installment"] = st.number_input("Monthly Installment (₹)", 100, 1_000_000, 5_000, step=100)

    elif selected_product in ACCOUNT_PRODUCTS:
        with c1:
            profile["customer_type"]    = st.selectbox("Customer Type", ["Individual", "NRI", "Minor (Guardian-operated)", "HUF"])
            profile["age"]              = st.number_input("Age (years)", 0, 120, 30)
        with c2:
            profile["kyc_documents"]    = st.multiselect("KYC Documents Available", ["Aadhaar", "PAN", "Passport", "Voter ID", "Driving License", "Utility Bill"], default=["Aadhaar", "PAN"])
        with c3:
            profile["annual_income"]    = st.number_input("Annual Income (₹)", 0, 100_000_000, 600_000, step=10_000)

        if selected_product == "current_account":
            c4, c5 = st.columns(2)
            with c4:
                profile["business_type"]    = st.selectbox("Business Type", ["Proprietorship", "Partnership", "Private Ltd", "LLP", "Trust / NGO"])
            with c5:
                profile["business_vintage_years"] = st.number_input("Business Vintage (years)", 0, 100, 3)

        elif selected_product == "salary_account":
            c4, c5 = st.columns(2)
            with c4:
                profile["employer_name"]    = st.text_input("Employer Name")
                profile["employer_tie_up"]  = st.selectbox("Employer Tie-Up with FinTrust?", ["Yes", "No", "Unknown"])
            with c5:
                profile["monthly_salary"]   = st.number_input("Monthly Salary (₹)", 0, 10_000_000, 50_000, step=1_000)

    elif selected_product in CARD_PRODUCTS:
        with c1:
            profile["age"]              = st.number_input("Age (years)", 18, 75, 30)
            profile["employment_type"]  = st.selectbox("Employment Type", ["Salaried", "Self-Employed Professional", "Self-Employed Business"])
        with c2:
            profile["monthly_income"]   = st.number_input("Monthly Income (₹)", 0, 10_000_000, 60_000, step=5_000)
            profile["cibil_score"]      = st.number_input("CIBIL Score", 300, 900, 700)
        with c3:
            if selected_product == "credit_card":
                profile["existing_credit_cards"] = st.number_input("Existing Credit Cards", 0, 20, 1)
                profile["card_type_requested"]   = st.selectbox("Card Type", ["Everyday", "Classic", "Premium", "Super Premium", "Business"])
            elif selected_product == "debit_card":
                profile["savings_account_held"]  = st.selectbox("Holds FinTrust Savings Account?", ["Yes", "No"])
                profile["account_status"]        = st.selectbox("Account Status", ["Active", "Inactive", "Frozen"])

    # Additional notes
    notes = st.text_area("Additional notes / special circumstances (optional)", height=70)
    if notes.strip():
    ...
    with run_col:
        run_button = st.button("🔍 Check Eligibility", type="primary", use_container_width=True)
    ...
    if run_button:
        if not profile:
            ...
            # Persist result so the HITL section survives reruns triggered by
            # the "Finalise" button click.
            st.session_state["elig_result"]        = result
            st.session_state["elig_product_label"] = selected_label
            st.session_state["elig_finalised"]     = None   # reset on new check

    # ── Show results from session state (survives button reruns) ─────────────
    ...
    if elig_result:
        ...
        decision_labels = {"PASS": "ELIGIBLE", "CONDITIONAL": "CONDITIONALLY ELIGIBLE",
        ...
        # Show finalised banner if already saved this session
        if st.session_state.get("elig_finalised"):
            ...
            with finalize_col:
                if st.button("📝 Finalise & Save Decision", use_container_width=True,
                             key="elig_finalise_btn"):
                    final = (officer_decision if officer_decision != "Accept AI Decision"
                             else decision)
                    # Map radio label → clean decision string
                    final_clean = (
...
with tab_checklist:
    ...
    # ── fetch product list (reuse from eligibility) ──────────────────────────
    @st.cache_data(ttl=300)
    def _fetch_checklist_products():
        try:
            r = requests.get(f"{API_BASE}/eligibility/products", timeout=5)
            return r.json().get("products", [])
        except Exception:
    ...
    cl_products    = _fetch_checklist_products()
    ...
    if cl_run:
            ...
            with st.expander(
                ...
                for item in items:
                    ...
                    if item.get("alternatives"):
                        alt_text = f"<br><span style='font-size:12px;color:#1565C0'>✦ Acceptable alternatives: {' &nbsp;|&nbsp; '.join(item['alternatives'])}</span>"
                    notes_text = ""
                    if item.get("notes"):
                        notes_text = f"<br><span style='font-size:12px;color:#555'>📝 {item['notes']}</span>"
        ...
        lines = [
            f"FINTRUST BANK — DOCUMENT CHECKLIST",
...
# ═══════════════════════════════════════════════════════════════════════════════
# TAB 4 — Fee and Interest Calculator
# ═══════════════════════════════════════════════════════════════════════════════

with tab_calc:
    st.subheader("🧮 Fee & Interest Calculator")
    st.caption(
    ...
    # ── Fetch product definitions (includes input specs) ─────────────────────
    @st.cache_data(ttl=300)
    ...
    if calc_run:
        ...
        if results:
            st.markdown("---")
            lines = [
                f"FINTRUST BANK — FEE & INTEREST CALCULATION",
...
with tab_compare:
    st.subheader("⚖️ Policy Comparison Tool")
    st.caption(
    ...
    # ── Manual product selectors ─────────────────────────────────────────────
    st.markdown("#### 🔧 Custom Comparison")
    col_a, col_vs, col_b = st.columns([5, 1, 5])

    # Resolve defaults from session state (set by quick-pick buttons)
    ...
    if cmp_run:
        ...
        # Colour coding: A wins → blue, B wins → purple, neutral/depends → grey
        ...
        st.markdown("---")
        export_lines = [
            f"FINTRUST BANK — POLICY COMPARISON",
            f"Product A : {label_a}",
            f"Product B : {label_b}",
            f"{'=' * 70}",
            "",
